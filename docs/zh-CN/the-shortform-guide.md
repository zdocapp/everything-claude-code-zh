...
**自 2 月实验性推出以来一直是 Claude Code 的忠实用户，并与 [@DRodriguezFX](https://x.com/DRodriguezFX) 一起，完全使用 Claude Code 在 Anthropic x Forum Ventures 黑客马拉松中凭借 [zenith.chat](https://zenith.chat) 获胜。**

这是我经过 10 个月日常使用后的完整设置：技能、钩子、子代理、MCP、插件，以及真正有效的东西。

***

## 技能与命令

技能是主要的工作流界面。它们就像作用域内的工作流包：可重用的提示、结构、支持文件，以及当你需要特定执行模式时的代码地图。

在用 Opus 4.5 进行长时间编码会话后，你想清理死代码和零散的 .md 文件吗？运行 `/refactor-clean`。需要测试？`/tdd`、`/e2e`、`/test-coverage`。这些斜杠命令条目很方便，但真正持久的单元是底层的技能。技能也可以包含代码地图——一种让 Claude 快速浏览你的代码库而无需消耗上下文进行探索的方式。

![显示链式命令的终端](../../assets/images/shortform/02-chaining-commands.jpeg)
*将命令链接在一起*

ECC 仍然附带一个 `commands/` 层，但最好将其视为迁移期间的遗留斜杠命令条目兼容性。持久的逻辑应该存在于技能中。
...
钩子是基于触发器的自动化程序，在特定事件发生时触发。与技能不同，它们仅限于工具调用和生命周期事件。
...
子代理是你的编排器（主 Claude）可以委托任务给其执行的进程，这些进程具有有限的作用域。它们可以在后台或前台运行，为主代理释放上下文。

子代理与技能配合得很好——一个能够执行你部分技能的子代理可以被委托任务并自主使用这些技能。它们也可以用特定的工具权限进行沙盒化。
...
为每个子代理配置允许的工具、MCP 和权限，以实现适当的作用域限制。
...
MCP 将 Claude 直接连接到外部服务。它不是 API 的替代品——它是一个围绕 API 的提示驱动包装器，允许在导航信息时具有更大的灵活性。

**示例：** Supabase MCP 允许 Claude 拉取特定数据，直接在上游运行 SQL 而无需复制粘贴。数据库、部署平台等也是如此。
...
**经验法则：** 在配置中保留 20-30 个 MCP，但保持启用状态的不超过 10 个 / 活动工具不超过 80 个。
...
插件将工具打包以便于安装，而不是繁琐的手动设置。一个插件可以是技能和 MCP 的组合，也可以是捆绑在一起的钩子/工具。
...
* **分支** (`/fork`) - 分支对话以并行执行不重叠的任务，而不是发送排队消息
* **Git Worktrees** - 用于重叠的并行 Claude 实例而不会产生冲突。每个 worktree 都是一个独立的检出
...
`mgrep` 是 ripgrep/grep 的显著改进。通过插件市场安装，然后使用 `/mgrep` 技能。适用于本地搜索和网络搜索。
...
* `/rewind` - 回到之前的状态
* `/statusline` - 使用分支、上下文百分比、待办事项进行自定义
...
你的编辑器选择会显著影响 Claude Code 工作流。虽然 Claude Code 可以在任何终端中工作，但将其与功能强大的编辑器配对可以解锁实时文件跟踪、快速导航和集成的命令执行。

### Zed（我的偏好）

我使用 [Zed](https://zed.dev) —— 用 Rust 编写，所以它真的很快。瞬间打开，轻松处理大型代码库，几乎不占用系统资源。

**为什么 Zed + Claude Code 是绝佳组合：**

* **速度** - 基于 Rust 的性能意味着当 Claude 快速编辑文件时不会出现延迟。你的编辑器能跟上
* **代理面板集成** - Zed 的 Claude 集成让你可以实时跟踪 Claude 编辑时的文件更改。无需离开编辑器即可跳转到 Claude 引用的文件
* **CMD+Shift+R 命令面板** - 快速访问所有自定义斜杠命令、调试器、构建脚本，界面可搜索
* **资源占用最小** - 在繁重操作期间不会与 Claude 竞争 RAM/CPU。这在运行 Opus 时很重要
* **Vim 模式** - 完整的 vim 键绑定，如果你喜欢的话

![带有自定义命令的 Zed 编辑器](../../assets/images/shortform/09-zed-editor.jpeg)
*使用 CMD+Shift+R 显示自定义命令下拉菜单的 Zed 编辑器。跟随模式显示为右下角的靶心图标。*

**编辑器无关提示：**

1. **分割屏幕** - 一侧是带有 Claude Code 的终端，另一侧是编辑器
2. **Ctrl + G** - 在 Zed 中快速打开 Claude 当前正在处理的文件
3. **自动保存** - 启用自动保存，以便 Claude 的文件读取始终是最新的
4. **Git 集成** - 使用编辑器的 Git 功能在提交前审查 Claude 的更改
5. **文件监视器** - 大多数编辑器会自动重新加载更改的文件，请确认此功能已启用

### VSCode / Cursor

这也是一个可行的选择，并且与 Claude Code 配合良好。你可以使用终端格式，通过 `\ide` 启用 LSP 功能（现在与插件有些冗余）实现与编辑器的自动同步。或者你可以选择扩展，它与编辑器集成度更高，并且具有匹配的 UI。
...
这是关键——我配置了 14 个 MCP，但每个项目只启用大约 5-6 个。保持上下文窗口健康。
...
*在纽约 Anthropic x Forum Ventures 黑客马拉松中，与 [@DRodriguezFX](https://x.com/DRodriguezFX) 一起构建 [zenith.chat](https://zenith.chat) 获胜*
