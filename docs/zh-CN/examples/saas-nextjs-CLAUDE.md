...
> 一个 Next.js + Supabase + Stripe SaaS 应用的现实示例。
> 将此复制到你的项目根目录，并根据你的技术栈进行自定义。

## 项目概述

**技术栈:** Next.js 15 (App Router), TypeScript, Supabase (认证 + 数据库), Stripe (计费), Tailwind CSS, Playwright (E2E)
...
* 所有查询都使用启用了 RLS 的 Supabase 客户端 — 绝不绕过 RLS
...
* 受保护的路由检查 `getUser()` — 绝不单独信任 `getSession()` 进行认证
* `middleware.ts` 中的中间件会在每个请求上刷新认证令牌

### 计费

* Stripe webhook 处理器在 `app/api/webhooks/stripe/route.ts` 中
* 绝不信任客户端的价格数据 — 始终在服务器端从 Stripe 获取
...
* 代码或注释中不使用表情符号
* 仅使用不可变模式 — 扩展运算符，绝不直接修改
* 服务器组件: 不使用 `'use client'` 指令，不使用 `useState`/`useEffect`
* 客户端组件: `'use client'` 放在顶部，保持最少 — 将逻辑提取到钩子中
...
* 功能分支从 `main` 创建，需要 PR
* CI 运行: 代码检查、类型检查、单元测试、E2E 测试
* 部署: PR 时部署到 Vercel 预览环境，合并到 `main` 时部署到生产环境
