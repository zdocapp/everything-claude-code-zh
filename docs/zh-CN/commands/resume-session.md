...
/resume-session ~/.claude/session-data/2024-01-15-abc123de-session.tmp  # 加载当前短ID会话文件
...
**会话文件引用了不再存在的文件：**
在简报中注明此情况 — "WARNING: `path/to/file.ts` referenced in session but not found on disk."

**会话文件来自超过 7 天前：**
注明时间间隔 — "WARNING: This session is from N days ago (threshold: 7 days). Things may have changed." — 然后正常进行。
...
不应重试的内容:
失败: Next-Auth — 与自定义 Prisma 适配器冲突, 每个请求都抛出适配器错误
...
然后使用 Postman 测试响应中是否包含 Set-Cookie 头。
