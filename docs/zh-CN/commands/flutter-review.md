...
[HIGH] BuildContext 在异步间隙后使用，未检查 mounted 状态
文件：lib/features/auth/presentation/login_page.dart:67
问题：在 `await auth.login(...)` 后调用了 `context.go('/home')`，没有 `mounted` 检查。
修复：在 await 之后的任何导航操作前添加 `if (!context.mounted) return;`（适用于 Flutter 3.7+）。

[HIGH] 未处理 AsyncValue 错误状态
文件：lib/features/auth/presentation/login_page.dart:42
问题：`ref.watch(authProvider)` 对 loading/data 进行了 switch 分支，但没有 `error` 分支。
