...
5. **质量门 — 检查清单 + 整体裁决**
   ...
   ### 5a. 必需检查清单（通过实际阅读文件来验证）

   在评估草案前，执行**所有**以下操作：

   * \[ ] 通过关键词在 `~/.claude/skills/` 和相关项目 `.claude/skills/` 文件中进行 grep 检查，查看内容重叠情况
   * \[ ] 检查 MEMORY.md（包括项目和全局）是否存在重叠
   * \[ ] 考虑是否追加到现有技能即可满足需求
   * \[ ] 确认这是一个可复用的模式，而非一次性修复

   ### 5b. 整体裁决

   综合检查清单结果和草案质量，然后选择**以下一项**：

   | 裁决 | 含义 | 后续操作 |
   |---------|---------|-------------|
   | **保存** | 独特、具体、范围明确 | 进入第 6 步 |
   | **改进后保存** | 有价值但需要改进 | 列出改进项 → 修订 → 重新评估（一次） |
   | **吸收到 \[X]** | 应追加到现有技能 | 显示目标技能和添加内容 → 第 6 步 |
   | **丢弃** | 琐碎、冗余或过于抽象 | 解释理由并停止 |
...
* **改进后保存**：呈现必需的改进项 + 修订后的草案 + 重新评估后的更新检查清单/裁决；如果修订后的裁决为**保存**，则在用户确认后保存，否则遵循新的裁决
* **保存**：呈现保存路径 + 检查清单结果 + 一行裁决理由 + 完整草案 → 用户确认后保存
* **吸收到 \[X]**：呈现目标路径 + 添加内容（差异格式）+ 检查清单结果 + 裁决理由 → 用户确认后追加
* **丢弃**：仅显示检查清单结果 + 理由（无需确认）
