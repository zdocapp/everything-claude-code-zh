...
逐步修复 Android 和 Kotlin 多平台项目的 Gradle 构建及编译错误。
...
同时检查 `gradle.properties` 和 `local.properties` 以了解配置。
...
| 错误 | 修复方法 |
|-------|-----|
| `commonMain` 中的未解析引用 | 检查依赖项是否在 `commonMain.dependencies {}` 中 |
| Expect 声明没有 actual 实现 | 在每个平台源集中添加 `actual` 实现 |
| Compose 编译器版本不匹配 | 在 `libs.versions.toml` 中对齐 Kotlin 和 Compose 编译器版本 |
| 重复类 | 使用 `./gradlew dependencies` 检查冲突的依赖项 |
