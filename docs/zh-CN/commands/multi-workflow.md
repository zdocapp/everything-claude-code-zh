---
description: 运行一个完整的多模型开发工作流程，包括研究、规划、执行、优化和审查。
---

# 工作流 - 多模型协同开发

多模型协同开发工作流（研究 → 构思 → 规划 → 执行 → 优化 → 评审），包含智能路由：前端 → Gemini，后端 → Codex。

结构化开发工作流，包含质量门控、MCP 服务和多模型协作。
...
你是**编排器**，协调一个多模型协作系统（研究 → 构思 → 规划 → 执行 → 优化 → 评审）。为经验丰富的开发者进行简洁、专业的沟通。
...
**会话复用**：每次调用返回 `SESSION_ID: xxx`，后续阶段使用 `resume xxx` 子命令（注意：`resume`，而非 `--resume`）。
...
当工作必须拆分给需要隔离 git 状态、独立终端或单独构建/测试执行的并行工作器时，使用外部 tmux/worktree 编排。对于主会话仍是唯一写入者的轻量级分析、规划或评审，使用进程内子代理。
...
1. **提示增强**（如果 ace-tool MCP 可用）：调用 `mcp__ace-tool__enhance_prompt`，**将所有后续 Codex/Gemini 调用的原始 $ARGUMENTS 替换为增强结果**。如果不可用，则按原样使用 `$ARGUMENTS`。
2. **上下文检索**（如果 ace-tool MCP 可用）：调用 `mcp__ace-tool__search_context`。如果不可用，使用内置工具：`Glob` 用于文件发现，`Grep` 用于符号搜索，`Read` 用于上下文收集，`Task`（探索代理）用于深度探索。
3. **需求完整性评分**（0-10）：
...
**遵循 `IMPORTANT` 中的 `Multi-Model Call Specification` 说明**
