...
Failure: `getById` 在应返回 null 时返回了一个用户（针对缺失的 ID）

Root cause: 测试设置添加了一个 ID 为 'test-id' 的用户，但查询时使用了 'missing-id'。
Fix: 更新测试以使用 'missing-id' 进行查询 —— 设置是正确的，查询是错误的。
...
Root cause: 部件中的空状态消息已从 'Your cart is empty' 重命名为 'Cart is empty'。
Fix: 更新测试字符串以匹配当前部件文案。
...
| `Golden file not found` | 运行 `flutter test --update-goldens` 以生成 |
