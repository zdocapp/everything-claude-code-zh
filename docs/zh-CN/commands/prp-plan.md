...
**核心理念**：一个优秀的计划应包含实现所需的一切，无需再提出更多问题。每一个模式、每一个约定、每一个陷阱——现在捕获，全程引用。
...
| 指向包含"实施阶段"的 `.md` 的路径 | 类 PRD 文档 | 解析阶段，找到下一个待处理阶段 |
...
4. **日志记录模式** — 识别记录什么内容、在什么级别记录以及以什么格式记录。
...
### 无需先验知识测试
不熟悉此代码库的开发人员应能仅使用此计划实施功能，而无需搜索代码库或提问。如果不能，请添加缺失的上下文。
