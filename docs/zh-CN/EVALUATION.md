...
运行 `ecc install --profile core` 以获得：
...
运行 `ecc install --profile full` 以获得全部 28 个智能体、116 个技能和 59 个命令。
...
* `stop-hook-git-check.sh` Stop 钩子具有生产级质量，并且已经强制执行了良好的 git 卫生习惯
* `Skill` 权限配置正确
* 配置干净，没有冲突或冗余
...
当前配置本质上是一个只有一个实现良好的 git 卫生钩子的空白状态。本仓库提供了一个完整的、经过生产测试的增强层，涵盖智能体、技能、命令、钩子和规则——并配有选择性安装系统，因此您可以精确添加所需内容，而不会使配置变得臃肿。
