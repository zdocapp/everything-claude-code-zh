...
仓库提供了 5 个安装配置文件。根据你的主要用例选择：

### 配置文件：`core` (最小可行设置)

> 安装最快。为你提供命令、核心代理、钩子运行时和质量工作流。

**最适合：** 尝试 ECC，占用空间最小，或环境受限的情况。
...
`rules/common/` 目录提供了在每次会话中生效的基础指导原则：
...
分支已经包含 EVALUATION.md 和 REPO-ASSESSMENT.md 文档 — 这对于一个工作分支来说没问题。

### 选项 C：从 npm 安装 (新机器最简单的方法)
...
| 需要考虑其他分支吗？ | 在此环境中未见；上游 `affaan-m/everything-claude-code` 是事实来源 |
| 最佳安装配置文件？ | `developer` 用于日常开发工作 |
| 当前设置的最大差距？ | 安装了 0 个代理 — 至少添加：planner, code-reviewer, tdd-guide, security-reviewer |
| 最快的收获？ | 运行 `node scripts/install-plan.js --profile core && node scripts/install-apply.js` |
