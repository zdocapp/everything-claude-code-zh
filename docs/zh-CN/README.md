...
### v1.10.0 — 界面刷新、操作员工作流和 ECC 2.0 Alpha 版 (2026年4月)

* **仪表板 GUI** — 新的基于 Tkinter 的桌面应用程序 (`ecc_dashboard.py` 或 `npm run dashboard`)，具有深色/浅色主题切换、字体自定义以及标题栏和任务栏中的项目徽标。
* **公共界面与实时仓库同步** — 元数据、目录计数、插件清单和面向安装的文档现在与实际的开源界面匹配：38 个智能体、156 个技能和 72 个遗留命令垫片。
...
* **媒体和启动工具** — `manim-video`、`remotion-video-creation` 以及升级的社交发布界面，使技术说明和启动内容成为同一系统的一部分。
* **框架和产品界面增长** — `nestjs-patterns`、更丰富的 Codex/OpenCode 安装界面以及扩展的跨执行环境打包，使仓库在 Claude Code 之外也能使用。
* **ECC 2.0 alpha 版已包含在代码库中** — `ecc2/` 中的 Rust 控制平面原型现在可以本地构建，并暴露 `dashboard`、`start`、`sessions`、`status`、`stop`、`resume` 和 `daemon` 命令。它可作为 alpha 版使用，还不是通用版本。
...
> 警告：**重要提示：** Claude Code 插件无法自动分发 `rules`。请手动安装它们：
>
> 如果你的本地 Claude 设置被清空或重置，这并不意味着你需要重新购买 ECC。请从 `ecc list-installed` 开始，然后运行 `ecc doctor` 和 `ecc repair`，之后再重新安装任何内容。这通常可以恢复 ECC 管理的文件，而无需重建你的设置。如果问题出在 ECC 工具的账户或市场访问上，请单独处理账单/账户恢复。
...
**就这样！** 你现在可以访问 48 个代理、183 个技能和 79 个遗留命令垫片。
...
|   |-- article-writing/            # 以指定风格撰写长文，避免通用 AI 语调 (NEW)
...
|   |-- plankton-code-quality/      # 使用 Plankton hooks 进行编写时代码质量强制 (NEW)
|
...
|   |-- memory-persistence/       # 会话生命周期 hooks (长文指南)
...
将所需的 MCP 服务器定义从 `mcp-configs/mcp-servers.json` 复制到你在 `~/.claude/settings.json` 中的官方 Claude Code 配置中，或者如果你希望仓库本地访问 MCP，则复制到项目范围的 `.mcp.json` 中。
...
ECC 管理的安装和 Codex 同步流程将跳过或移除那些捆绑的服务器，而不是重新添加重复项。
...
技能是主要的工作流界面。它们可以直接调用、自动建议并被代理重用。ECC 在迁移期间仍会提供 `commands/`，但新的工作流开发应首先在 `skills/` 中进行。
...
不确定从哪里开始？使用此快速参考。技能是规范的工作流界面；下面的斜杠条目是大多数用户已经知道的兼容形式。
...
下面的斜杠形式之所以显示，是因为它们仍然是用户最熟悉的快速入口点。在底层，ECC 正在将这些工作流转向以技能为首的定义。
...
过多的 MCP 服务器会消耗你的上下文。每个 MCP 工具描述都会从你的 20 万窗口占用令牌，可能将其减少到约 7 万。
...
ECC 提供 **完整的 Cursor IDE 支持**，包含为 Cursor 原生格式适配的钩子、规则、代理、技能、命令和 MCP 配置。
...
| 规则 | 34 | 9 个通用规则（alwaysApply）+ 25 个特定语言规则（TypeScript、Python、Go、Swift、PHP） |
| 代理 | 共享 | 通过根目录的 AGENTS.md（由 Cursor 原生读取） |
| 技能 | 共享 + 捆绑 | 通过根目录的 AGENTS.md 和 `.cursor/skills/` 获取翻译后的附加内容 |
| 命令 | 共享 | 如果已安装，则使用 `.cursor/commands/` |
| MCP 配置 | 共享 | 如果已安装，则使用 `.cursor/mcp.json` |
...
| brand-voice | 基于真实内容的源衍生写作风格配置文件 |
| bun-runtime | 将 Bun 作为运行时、包管理器、打包器和测试运行器 |
| claude-api | 适用于 Python 和 TypeScript 的 Anthropic Claude API 模式 |
| coding-standards | 通用编码标准 |
| content-engine | 平台原生的社交内容创作和再利用 |
...
| 命令 | 通过：79 个命令 | 通过：31 个命令 | **Claude Code 领先** |
| 技能 | 通过：183 个技能 | 通过：37 个技能 | **Claude Code 领先** |
| 钩子 | 通过：8 种事件类型 | 通过：11 种事件 | **OpenCode 更多！** |
| 规则 | 通过：29 条规则 | 通过：13 条指令 | **Claude Code 领先** |
| MCP 服务器 | 通过：14 个服务器 | 通过：完整 | **完全对等** |
| 自定义工具 | 通过：通过钩子 | 通过：6 个原生工具 | **OpenCode 更好** |
...
| `/setup-pm` | 配置包管理器 |
| `/harness-audit` | 审计工具链可靠性、评估准备情况和风险状况 |
...
| **规则** | 34 条（通用 + 语言） | 34 条（YAML 前言） | 基于指令 | 13 条指令 |
| **自定义工具** | 通过钩子 | 通过钩子 | 不适用 | 6 个原生工具 |
| **MCP 服务器** | 14 | 共享（mcp.json） | 7（通过 TOML 解析器自动合并） | 完整支持 |
| **配置格式** | settings.json | hooks.json + rules/ | config.toml | opencode.json |
| **上下文文件** | CLAUDE.md + AGENTS.md | AGENTS.md | AGENTS.md | AGENTS.md |
| **秘密检测** | 基于钩子 | beforeSubmitPrompt 钩子 | 基于沙盒 | 基于钩子 |
