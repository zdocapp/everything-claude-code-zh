**语言：** 简体中文 | [Português (Brasil)](../../../pt-BR/README.md) | [English](../../../../README.md) | [繁體中文](../../../zh-TW/README.md) | [日本語](../../../ja-JP/README.md) | [한국어](../../../ko-KR/README.md) | [Türkçe](../../../tr/README.md)
...
> **14万+星标** | **2.1万+复刻** | **170+贡献者** | **12+语言生态系统** | **Anthropic黑客马拉松获奖者**
...
不仅仅是配置。一个完整的系统：技能、本能、内存优化、持续学习、安全扫描，以及研究优先的开发。经过10多个月密集的日常使用、构建真实产品的锤炼，演化出生产就绪的代理、技能、钩子、规则、MCP配置和遗留命令垫片。

可在 **Claude Code**、**Codex**、**Cursor**、**OpenCode**、**Gemini** 及其他AI智能体框架中运行。
...
### v1.10.0 — 界面刷新、操作员工作流和ECC 2.0 Alpha版（2026年4月）

* **公共界面与实时仓库同步** — 元数据、目录计数、插件清单和面向安装的文档现已与实际的开源界面匹配：38个智能体，156项技能，以及72个遗留命令垫片。
...
* **媒体和发布工具** — `manim-video`、`remotion-video-creation` 以及升级后的社交发布界面，使技术说明和发布内容成为同一系统的一部分。
* **框架和产品界面增长** — `nestjs-patterns`、更丰富的Codex/OpenCode安装界面，以及扩展的跨框架打包，使仓库在Claude Code之外也能使用。
* **ECC 2.0 alpha版已集成在代码树中** — `ecc2/` 中的Rust控制平面原型现已可在本地构建，并暴露 `dashboard`、`start`、`sessions`、`status`、`stop`、`resume` 和 `daemon` 命令。它可作为alpha版使用，尚非通用版本。
* **生态系统加固** — AgentShield、ECC Tools成本控制、计费门户工作以及网站刷新继续围绕核心插件发布，而非分散到独立的孤岛中。

### v1.9.0 — 选择性安装与语言扩展 (2026年3月)

* **选择性安装架构** — 清单驱动的安装管道，包含 `install-plan.js` 和 `install-apply.js`，用于目标组件安装。状态存储跟踪已安装内容并支持增量更新。
* **6个新智能体** — `typescript-reviewer`、`pytorch-build-resolver`、`java-build-resolver`、`java-reviewer`、`kotlin-reviewer`、`kotlin-build-resolver` 将语言覆盖范围扩展到10种语言。
* **新技能** — 用于深度学习工作流的 `pytorch-patterns`，用于API参考研究的 `documentation-lookup`，用于现代JS工具链的 `bun-runtime` 和 `nextjs-turbopack`，外加8个运营领域技能和 `mcp-server-patterns`。
* **会话与状态基础设施** — 带有查询CLI的SQLite状态存储、用于结构化记录的会话适配器、用于自我改进技能的技能演进基础。
* **编排系统大修** — 使框架审计评分具有确定性，强化编排状态和启动器兼容性，通过5层防护防止观察者循环。
* **观察者可靠性** — 通过节流和尾部采样修复内存爆炸问题，修复沙箱访问，实现延迟启动逻辑和重入防护。
* **12个语言生态系统** — Java、PHP、Perl、Kotlin/Android/KMP、C++和Rust的新规则加入现有的TypeScript、Python、Go和通用规则。
* **社区贡献** — 韩语和中文翻译、biome钩子优化、视频处理技能、运营技能、PowerShell安装程序、Antigravity IDE支持。
* **CI加固** — 修复了19个测试失败问题，强制执行目录计数，验证安装清单，并使完整测试套件全部通过。
...
* **修复本能导入内容丢失** — `parse_instinct_file()` 在 `/instinct-import` 期间静默丢弃了frontmatter之后的所有内容（行动、证据、示例部分）。 ([#148](https://github.com/affaan-m/everything-claude-code/issues/148), [#161](https://github.com/affaan-m/everything-claude-code/pull/161))
...
> 注意：该插件很方便，但如果您的Claude Code版本在解析自托管市场条目时遇到问题，下面的开源安装程序仍然是最可靠的途径。
...
> 警告：**重要：** Claude Code 插件无法自动分发 `rules`。请手动安装：
...
>
> 要使用 `/multi-plan`、`/multi-execute`、`/multi-backend`、`/multi-frontend` 和 `/multi-workflow`，您还必须安装 `ccg-workflow` 运行时。
>
...
|   |-- frontend-slides/            # HTML 幻灯片和 PPTX 转网页演示文稿工作流 (NEW)
|   |-- article-writing/            # 以指定风格进行长篇写作，避免通用 AI 语调 (NEW)
|   |-- content-engine/             # 多平台社交内容创作和再利用工作流 (NEW)
|   |-- market-research/            # 带来源引用的市场、竞争对手和投资者研究 (NEW)
|   |-- investor-materials/         # 融资演讲稿、一页纸简介、备忘录和财务模型 (NEW)
|   |-- investor-outreach/          # 个性化的融资外联和跟进 (NEW)
|   |-- continuous-learning/        # 从会话中自动提取模式 (Longform Guide)
|   |-- continuous-learning-v2/     # 基于直觉的学习，带置信度评分
|   |-- iterative-retrieval/        # 用于子代理的渐进式上下文精炼
|   |-- strategic-compact/          # 手动压缩建议 (Longform Guide)
|   |-- tdd-workflow/               # TDD 方法论
|   |-- security-review/            # 安全检查清单
|   |-- eval-harness/               # 验证循环评估 (Longform Guide)
|   |-- verification-loop/          # 持续验证 (Longform Guide)
|   |-- videodb/                   # 视频和音频：摄取、搜索、编辑、生成、流式传输 (NEW)
|   |-- golang-patterns/            # Go 惯用法和最佳实践
|   |-- golang-testing/             # Go 测试模式、TDD、基准测试
|   |-- cpp-coding-standards/         # 来自 C++ Core Guidelines 的 C++ 编码标准 (NEW)
|   |-- cpp-testing/                # 使用 GoogleTest、CMake/CTest 的 C++ 测试 (NEW)
|   |-- django-patterns/            # Django 模式、模型、视图 (NEW)
|   |-- django-security/            # Django 安全最佳实践 (NEW)
|   |-- django-tdd/                 # Django TDD 工作流 (NEW)
|   |-- django-verification/        # Django 验证循环 (NEW)
|   |-- laravel-patterns/           # Laravel 架构模式 (NEW)
|   |-- laravel-security/           # Laravel 安全最佳实践 (NEW)
|   |-- laravel-tdd/                # Laravel TDD 工作流 (NEW)
|   |-- laravel-verification/       # Laravel 验证循环 (NEW)
|   |-- python-patterns/            # Python 惯用法和最佳实践 (NEW)
|   |-- python-testing/             # 使用 pytest 的 Python 测试 (NEW)
|   |-- springboot-patterns/        # Java Spring Boot 模式 (NEW)
|   |-- springboot-security/        # Spring Boot 安全 (NEW)
|   |-- springboot-tdd/             # Spring Boot TDD (NEW)
|   |-- springboot-verification/    # Spring Boot 验证 (NEW)
|   |-- configure-ecc/              # 交互式安装向导 (NEW)
|   |-- security-scan/              # AgentShield 安全审计器集成 (NEW)
|   |-- java-coding-standards/     # Java 编码标准 (NEW)
|   |-- jpa-patterns/              # JPA/Hibernate 模式 (NEW)
|   |-- postgres-patterns/         # PostgreSQL 优化模式 (NEW)
|   |-- nutrient-document-processing/ # 使用 Nutrient API 进行文档处理 (NEW)
|   |-- docs/examples/project-guidelines-template.md  # 项目特定技能模板
|   |-- database-migrations/         # 迁移模式 (Prisma, Drizzle, Django, Go) (NEW)
|   |-- api-design/                  # REST API 设计、分页、错误响应 (NEW)
|   |-- deployment-patterns/         # CI/CD、Docker、健康检查、回滚 (NEW)
|   |-- docker-patterns/            # Docker Compose、网络、卷、容器安全 (NEW)
|   |-- e2e-testing/                 # Playwright E2E 模式和页面对象模型 (NEW)
|   |-- content-hash-cache-pattern/  # 用于文件处理的 SHA-256 内容哈希缓存 (NEW)
|   |-- cost-aware-llm-pipeline/     # LLM 成本优化、模型路由、预算跟踪 (NEW)
|   |-- regex-vs-llm-structured-text/ # 决策框架：文本解析用正则表达式 vs LLM (NEW)
|   |-- swift-actor-persistence/     # 使用 actors 实现线程安全的 Swift 数据持久化 (NEW)
|   |-- swift-protocol-di-testing/   # 基于协议的 DI 实现可测试的 Swift 代码 (NEW)
|   |-- search-first/               # 先研究再编码的工作流 (NEW)
|   |-- skill-stocktake/            # 审计技能和命令的质量 (NEW)
|   |-- liquid-glass-design/         # iOS 26 Liquid Glass 设计系统 (NEW)
|   |-- foundation-models-on-device/ # 使用 FoundationModels 的 Apple 设备端 LLM (NEW)
|   |-- swift-concurrency-6-2/       # Swift 6.2 易于理解的并发 (NEW)
|   |-- perl-patterns/             # 现代 Perl 5.36+ 惯用法和最佳实践 (NEW)
|   |-- perl-security/             # Perl 安全模式、污染模式、安全 I/O (NEW)
|   |-- perl-testing/              # 使用 Test2::V0、prove、Devel::Cover 的 Perl TDD (NEW)
|   |-- autonomous-loops/           # 自主循环模式：顺序管道、PR 循环、DAG 编排 (NEW)
|   |-- plankton-code-quality/      # 使用 Plankton 钩子进行编写时代码质量强制 (NEW)
|
|-- commands/         # 遗留的斜杠命令入口垫片；推荐使用 skills/
|   |-- tdd.md              # /tdd - 测试驱动开发
|   |-- plan.md             # /plan - 实现规划
|   |-- e2e.md              # /e2e - E2E 测试生成
...
|   |-- learn-eval.md       # /learn-eval - 提取、评估和保存模式 (NEW)
...
|   |-- skill-create.md     # /skill-create - 从 git 历史生成技能 (NEW)
|   |-- instinct-status.md  # /instinct-status - 查看已学习的直觉 (NEW)
|   |-- instinct-import.md  # /instinct-import - 导入直觉 (NEW)
|   |-- instinct-export.md  # /instinct-export - 导出直觉 (NEW)
|   |-- evolve.md           # /evolve - 将直觉聚类为技能
|   |-- prune.md            # /prune - 删除过期的待处理直觉 (NEW)
|   |-- pm2.md              # /pm2 - PM2 服务生命周期管理 (NEW)
|   |-- multi-plan.md       # /multi-plan - 多代理任务分解 (NEW)
|   |-- multi-execute.md    # /multi-execute - 编排的多代理工作流 (NEW)
|   |-- multi-backend.md    # /multi-backend - 后端多服务编排 (NEW)
|   |-- multi-frontend.md   # /multi-frontend - 前端多服务编排 (NEW)
|   |-- multi-workflow.md   # /multi-workflow - 通用多服务工作流 (NEW)
|   |-- orchestrate.md      # /orchestrate - 多代理协调
|   |-- sessions.md         # /sessions - 会话历史管理
|   |-- eval.md             # /eval - 根据标准进行评估
|   |-- test-coverage.md    # /test-coverage - 测试覆盖率分析
|   |-- update-docs.md      # /update-docs - 更新文档
|   |-- update-codemaps.md  # /update-codemaps - 更新代码地图
|   |-- python-review.md    # /python-review - Python 代码审查 (NEW)
|
...
|   |-- memory-persistence/       # 会话生命周期钩子 (Longform Guide)
...
|   |-- run-all.js               # 运行所有测试
|
|-- contexts/         # 动态系统提示注入上下文 (Longform Guide)
|   |-- dev.md              # 开发模式上下文
|   |-- review.md           # 代码审查模式上下文
...
|   |-- laravel-api-CLAUDE.md     # 真实世界 Laravel API (PostgreSQL + Redis) (NEW)
|   |-- rust-api-CLAUDE.md        # 真实世界 Rust API (Axum + SQLx + PostgreSQL) (NEW)
|
...
> 警告：**致贡献者：** 请勿向 `.claude-plugin/plugin.json` 添加 `"hooks"` 字段。这由回归测试强制执行。
...
从 `mcp-configs/mcp-servers.json` 复制所需的MCP服务器定义到你在 `~/.claude/settings.json` 中的官方Claude Code配置中，或者复制到项目作用域的 `.mcp.json` 中，如果你想要仓库本地的MCP访问权限。

如果您已经运行了自己版本的ECC捆绑的MCP服务器，请设置：

```bash
export ECC_DISABLED_MCPS="github,context7,exa,playwright,sequential-thinking,memory"
```

ECC管理的安装和Codex同步流程将跳过或移除那些捆绑的服务器，而不是重新添加重复项。
...
技能是主要的工作流界面。它们可以被直接调用、自动建议，并被代理复用。ECC在迁移期间仍提供 `commands/`，但新的工作流开发应首先落地在 `skills/` 中。
...
不确定从哪里开始？使用这个快速参考。技能是规范的工作流界面；下面的斜杠条目是大多数用户已经知道的兼容形式。
...
| 审查Go代码 | `/go-review` | go-reviewer |
| 审查Python代码 | `/python-review` | python-reviewer |
| 审查TypeScript/JavaScript代码 | *(直接调用 `typescript-reviewer`)* | typescript-reviewer |
| 审计数据库查询 | *(自动委派)* | database-reviewer |

### 常见工作流

下面显示斜杠形式，因为它们仍然是用户最熟悉、最快的入口点。在底层，ECC正在将这些工作流转向技能优先的定义。
...
* **Cursor**: `.cursor/` 中的预翻译配置。参见 [Cursor IDE支持](#cursor-ide-支持)。
* **Gemini CLI**: 通过 `.gemini/GEMINI.md` 和共享的安装程序管道实现实验性的项目本地支持。
* **OpenCode**: `.opencode/` 中的完整插件支持。参见 [OpenCode支持](#opencode-支持)。
* **Codex**: 对macOS应用和CLI的一流支持，包含适配器漂移防护和SessionStart回退。参见PR [#257](https://github.com/affaan-m/everything-claude-code/pull/257)。
* **Antigravity**: `.agent/` 中为工作流、技能和扁平化规则紧密集成的设置。参见 [Antigravity指南](../ANTIGRAVITY-GUIDE.md)。
* **非原生框架**: 用于Grok及类似界面的手动回退路径。参见 [手动适配指南](../MANUAL-ADAPTATION-GUIDE.md)。
...
* 特定框架配置（Rails, FastAPI）—— Django, NestJS, Spring Boot, 和 Laravel 已包含在内
...
同步脚本使用**仅添加**策略将 ECC MCP 服务器安全地合并到您现有的 `~/.codex/config.toml` 中 — 它从不删除或修改您现有的服务器。使用 `--dry-run` 运行以预览更改，或使用 `--update-mcp` 强制将 ECC 服务器刷新到最新的推荐配置。

对于 Context7，ECC 使用规范的 Codex 部分名称 `[mcp_servers.context7]`，同时仍然启动 `@upstash/context7-mcp` 包。如果您已有旧的 `[mcp_servers.context7-mcp]` 条目，`--update-mcp` 会将其迁移到规范的部分名称。
...
| 组件 | 数量 | 详情 |
|-----------|-------|---------|
| 配置 | 1 | `.codex/config.toml` — 顶层 approvals/sandbox/web\_search、MCP服务器、通知、配置文件 |
| AGENTS.md | 2 | 根目录（通用）+ `.codex/AGENTS.md`（Codex特定补充） |
| 技能 | 30 | `.agents/skills/` — 每个技能的SKILL.md + agents/openai.yaml |
| MCP服务器 | 6 | GitHub、Context7、Exa、Memory、Playwright、Sequential Thinking（通过 `--update-mcp` 同步Supabase后为7个） |
...
| backend-patterns | API设计、数据库、缓存 |
| brand-voice | 基于真实内容派生的写作风格配置文件 |
| bun-runtime | 将Bun作为运行时、包管理器、打包器和测试运行器 |
| claude-api | 用于Python和TypeScript的Anthropic Claude API模式 |
| coding-standards | 通用编码标准 |
| content-engine | 平台原生的社交内容和再利用 |
| crosspost | 跨X、LinkedIn、Threads的多平台内容分发 |
| deep-research | 多源研究，包含综合和来源归属 |
| dmux-workflows | 使用tmux窗格管理器进行多智能体编排 |
| documentation-lookup | 通过Context7 MCP获取最新的库和框架文档 |
| e2e-testing | Playwright端到端测试 |
| eval-harness | 基于评估的开发 |
| everything-claude-code | 项目的开发约定和模式 |
| exa-search | 通过Exa MCP进行网络、代码、公司研究的神经搜索 |
| fal-ai-media | 用于图像、视频和音频的统一媒体生成 |
| frontend-patterns | React/Next.js模式 |
| frontend-slides | HTML演示文稿、PPTX转换、视觉风格探索 |
| investor-materials | 演示文稿、备忘录、模型和一页纸简介 |
...
| tdd-workflow | 测试驱动开发，覆盖率80%以上 |
| verification-loop | 构建、测试、代码检查、类型检查、安全 |
| video-editing | 使用FFmpeg和Remotion的AI辅助视频编辑工作流 |
| x-api | 用于发布和分析的X/Twitter API集成 |
...
| MCP 服务器 | PASS: 14 个 | PASS: 全部 | **完全对等** |
...
| `/multi-backend` | 后端导向的多模型工作流 |
| `/multi-frontend` | 前端导向的多模型工作流 |
...
| `/prune` | 删除已过期的待处理本能（30 天 TTL） |
| `/learn-eval` | 在保存前提取和评估模式 |
| `/setup-pm` | 配置包管理器 |
| `/harness-audit` | 审计harness可靠性、评估准备情况和风险状况 |
...
| **MCP 服务器** | 14 | 共享 (mcp.json) | 7 (基于命令) | 完整 |
...
我从实验性推出阶段就开始使用 Claude Code。在 2025 年 9 月与 [@DRodriguezFX](https://x.com/DRodriguezFX) 共同赢得了 Anthropic x Forum Ventures 黑客马拉松 — 完全使用 Claude Code 构建了 [zenith.chat](https://zenith.chat)。
...
基于Everything Claude Code构建或受其启发的项目：

| 项目 | 描述 |
|---------|-------------|
| [EVC](https://github.com/SaigonXIII/evc) | 营销智能体工作空间 — 为内容运营者、品牌治理和多渠道发布提供42个命令。[视觉概览](https://saigonxiii.github.io/evc)。 |

用ECC构建了什么吗？请提交PR将其添加到这里。
...
* **速成指南（从此开始）：** [Claude Code 万物速成指南](https://x.com/affaanmustafa/status/2012378465664745795)
* **长篇指南（高级）：** [Claude Code 万物长篇指南](https://x.com/affaanmustafa/status/2014040193557471352)
