...
* 仅将令牌、个人身份信息（PII）和凭证存储在 `flutter_secure_storage` 中
...
* 审查 Intent 过滤器——导出的组件若带有隐式 Intent 过滤器，则任何应用均可访问
* 对于显示敏感数据的屏幕，使用 `FLAG_SECURE`（防止截图）
...
* 使用 `webview_flutter` v4+（`WebViewController` / `WebViewWidget`）——旧版的 `WebView` 组件已被移除
...
* 将 `--split-debug-info` 输出排除在版本控制之外（仅用于崩溃符号化）
