---
description: 用于 e2e 测试技能的旧版斜杠入口填充程序。建议直接使用该技能。
---

# E2E 命令（旧版兼容层）

仅当您仍调用 `/e2e` 时使用此命令。维护的工作流位于 `skills/e2e-testing/SKILL.md`。

## 规范接口

* 优先直接使用 `e2e-testing` 技能。
* 仅将此文件保留为兼容性入口点。
...
* 除非用户明确要求运行整个测试套件，否则仅运行相关测试。
* 捕获常规工件并报告失败、不稳定风险和后续修复，无需在此处重复完整的技能主体。
...
使用 3 个工作进程运行 3 个测试
  ...
  ✓  [chromium] › search-and-view.spec.ts:5:3 › 用户可以搜索市场并查看详情 (4.2秒)
  ✓  [chromium] › search-and-view.spec.ts:52:3 › 无结果搜索显示空状态 (1.8秒)
  ✓  [chromium] › search-and-view.spec.ts:67:3 › 可以清除搜索并重新查看所有市场 (2.9秒)
