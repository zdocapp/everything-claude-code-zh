...
* 为 Hermes 操作符故事线添加了公开的 ECC 2.0 发布候选版本表面。
* 将 ECC 记录为跨 Claude Code、Codex、Cursor、OpenCode 和 Gemini 的可复用跨框架基础组件。
...
* 暂时保持 `ecc2/` 版本控制独立；除非发布工程团队另有决定，它仍是一个 alpha 控制平面脚手架。

### 备注

* 这是一个发布候选版本，并非对完整 ECC 2.0 控制平面路线图的 GA 声明。
* 预发布 npm 发布应使用 `next` 分发标签，除非发布工程团队明确选择其他方式。
