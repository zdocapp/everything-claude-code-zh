...
description: 通过Claude DevFleet编排多智能体编码任务——规划项目、在隔离的工作树中分派并行智能体、监控进度并阅读结构化报告。
origin: community
---
...
当需要调度多个 Claude Code 智能体并行处理编码任务时使用此技能。每个智能体在独立的 git 工作树中运行，并配备完整的工具集。

需要已连接 MCP 的 Claude DevFleet 实例正在运行：
...
用户 → "构建一个带有身份验证和测试的 REST API"
  ↓
plan_project(提示) → 项目ID + 任务DAG
  ↓
向用户展示计划 → 获取批准
  ↓
dispatch_mission(M1) → 代理1在工作树中生成
  ↓
M1完成 → 自动合并 → 自动分派M2 (依赖于M1)
  ↓
M2完成 → 自动合并
  ↓
get_report(M2) → 已更改的文件、完成的工作、错误、后续步骤
  ↓
向用户报告
```
...
| 工具 | 用途 |
|------|---------|
| `plan_project(prompt)` | AI 将描述分解为具有链式任务的项目 |
...
> **关于 `wait_for_mission` 的说明：** 此操作会阻塞对话，最长 `timeout_seconds` 秒（默认 600）。对于长时间运行的任务，建议每 30–60 秒使用 `get_mission_status` 轮询，以便用户能看到进度更新。

### 工作流程：规划 → 调度 → 监控 → 报告

1. **规划**：调用 `plan_project(prompt="...")` → 返回 `project_id` 及任务列表，包含 `depends_on` 链和 `auto_dispatch=true`。
2. **展示计划**：向用户展示任务标题、类型和依赖链。
3. **调度**：在根任务（`depends_on` 为空）上调用 `dispatch_mission(mission_id=<first_mission_id>)`。剩余任务在其依赖项完成时自动调度（因为 `plan_project` 为它们设置了 `auto_dispatch=true`）。
4. **监控**：调用 `get_mission_status(mission_id=...)` 或 `get_dashboard()` 检查进度。
5. **报告**：任务完成时调用 `get_report(mission_id=...)`。与用户分享重点内容。

### 并发性

DevFleet 默认最多同时运行 3 个智能体（可通过 `DEVFLEET_MAX_AGENTS` 配置）。当所有槽位都满时，具有 `auto_dispatch=true` 的任务会在任务监视器中排队，并在槽位空闲时自动调度。检查 `get_dashboard()` 了解当前槽位使用情况。
...
5. `create_mission(project_id=project_id, title="Review", prompt="...", depends_on=[impl_mission_id], auto_dispatch=true)` —— 由于依赖已满足，将自动启动。

## 指南

* 除非用户明确指示继续，否则在调度前务必与用户确认计划。
* 报告状态时包含任务标题和 ID。
* 如果任务失败，在重试前先读取其报告。
* 在批量调度前，检查 `get_dashboard()` 了解智能体槽位可用性。
* 任务依赖关系构成一个有向无环图（DAG）—— 不要创建循环依赖。
* 每个智能体在独立的 git 工作树中运行，并在完成时自动合并。如果发生合并冲突，更改将保留在智能体的工作树分支上，以便手动解决。
* 手动创建任务时，如果希望它们在依赖项完成时自动触发，请务必设置 `auto_dispatch=true`。没有此标志，任务将保持 `draft` 状态。
