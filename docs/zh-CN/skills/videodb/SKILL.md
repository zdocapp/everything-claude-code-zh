---
name: videodb
description: 查看、理解并处理视频和音频。查看——从本地文件、URL、RTSP/直播源或实时录制桌面获取内容；返回实时上下文和可播放流链接。理解——提取帧，构建视觉/语义/时间索引，并通过时间戳和自动剪辑搜索片段。处理——转码和标准化（编解码器、帧率、分辨率、宽高比），执行时间线编辑（字幕、文本/图像叠加、品牌标识、音频叠加、配音、翻译），生成媒体资产（图像、音频、视频），并为来自直播流或桌面捕获的事件创建实时警报。
origin: ECC
...
* 流式传输**实时上下文**并存储**阶段性会话记忆**
* 对所说内容和屏幕上发生的事件运行**实时警报/触发器**
...
* 连接 **RTSP/直播源**
* 运行**实时视觉和语音理解**并发出**事件/警报**以用于监控工作流
...
* 桌面捕获请求：**启动 / 停止 / 总结会话**
* 期望的操作：获取用于理解的上下文、转码规格、索引规格、搜索查询、剪辑范围、时间线编辑、警报规则
...
当简短的命令行内命令有效时，请**不要**编写脚本文件。

编写内联 Python 代码（`python -c "..."`）时，请始终使用格式正确的代码——使用分号分隔语句并保持可读性。对于任何超过约 3 条语句的内容，请改用 heredoc：
...
请**不要**自行读取、写入或处理 API 密钥。始终让用户自行设置。
...
| 场景 | 错误信息 | 解决方案 |
|------|----------|----------|
| 为已索引的视频建立索引 | `Spoken word index for video already exists` | 使用 `video.index_spoken_words(force=True)` 在已索引时跳过 |
...
5. **事件写入到**：`$STATE_DIR/videodb_events.jsonl`
...
参考文档位于与此 SKILL.md 文件相邻的 `reference/` 目录中。如果需要，请使用 Glob 工具来定位它。

* [reference/api-reference.md](reference/api-reference.md) - 完整的 VideoDB Python SDK API 参考
* [reference/search.md](reference/search.md) - 视频搜索深入指南（口语词和基于场景）
...
当 VideoDB 支持该操作时，**请勿使用 ffmpeg、moviepy 或本地编码工具**。以下所有操作均由 VideoDB 在服务器端处理——修剪、合并剪辑、叠加音频或音乐、添加字幕、文本/图像叠加层、转码、分辨率更改、宽高比转换、为平台要求调整大小、转录和媒体生成。仅当 reference/editor.md 中“限制”部分列出的操作（转场、速度变化、裁剪/缩放、色彩分级、音量混合）时，才回退到本地工具。

### 何时使用何种方案

| 问题 | VideoDB 解决方案 |
|------|-----------------|
| 平台拒绝视频宽高比或分辨率 | 带有 `VideoConfig` 的 `video.reframe()` 或 `conn.transcode()` |
...
此技能的参考材料在本地 `skills/videodb/reference/` 下提供。
请使用上述本地副本，而不是在运行时跟随外部仓库链接。
