...
视频**必须被索引**后才能进行搜索。索引是每种索引类型针对每个视频的一次性操作。

## 索引

### 口语词索引

索引视频的转录语音内容，用于语义搜索和关键词搜索：
...
当使用自定义元数据索引场景时，您可以将语义搜索与元数据过滤器结合：
...
有关自定义元数据索引和过滤搜索的完整示例，请参阅 [scene\_level\_metadata\_indexing 示例库](https://github.com/video-db/videodb-cookbook/blob/main/quickstart/scene_level_metadata_indexing.ipynb)。
...
> **注意：** 集合级别的搜索仅支持 `SearchType.semantic`。将 `SearchType.keyword` 或 `SearchType.scene` 与 `coll.search()` 一起使用将引发 `NotImplementedError`。要进行关键词或场景搜索，请改为在单个视频上使用 `video.search()`。

## 搜索 + 编译

索引、搜索并将匹配片段编译成单个可播放的流：
...
* **一次索引，多次搜索**：索引是昂贵的操作。一旦索引完成，搜索速度很快。
