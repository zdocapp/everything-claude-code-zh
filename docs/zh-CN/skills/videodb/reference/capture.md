...
有关代码级别的详细信息（SDK 方法、事件结构、AI 流水线），请参阅 [capture-reference.md](capture-reference.md)。

## 快速开始

1. **启动 WebSocket 监听器**：`python scripts/ws_listener.py --clear &`
2. **运行捕获代码**（参见下方完整捕获工作流程）
...
> **关键：** `CaptureClient` 必须在整个捕获期间保持运行。它运行本地录制器二进制文件，将屏幕/音频数据流式传输到 VideoDB。如果创建 `CaptureClient` 的 Python 进程退出，录制器二进制文件会被终止，捕获将静默停止。始终将捕获代码作为**长期运行的后台进程**运行（例如 `nohup python capture_script.py &`），并使用信号处理（`asyncio.Event` + `SIGINT`/`SIGTERM`）来保持其存活，直到您明确停止它。

1. 在后台**启动 WebSocket 监听器**，使用 `--clear` 标志来清除旧事件。等待其创建 WebSocket ID 文件。

2. **读取 WebSocket ID**。此 ID 是捕获会话和 AI 流水线所必需的。
...
8. **保持进程存活。** 使用 `asyncio.Event` 配合 `SIGINT`/`SIGTERM` 的信号处理器来阻塞进程，直到明确停止。写入一个 PID 文件（例如 `/tmp/videodb_capture_pid`），以便稍后可以使用 `kill $(cat /tmp/videodb_capture_pid)` 停止该进程。PID 文件应在每次运行时被覆盖，以便重新运行时始终具有正确的 PID。

9. **启动 AI 流水线**（在单独的命令/脚本中）对每个 RTStream 进行音频索引和视觉索引。从保存的会话信息文件中读取 RTStream ID。

10. **编写自定义事件处理逻辑**（在单独的命令/脚本中），根据您的用例读取实时事件。示例：
    * 当 `visual_index` 提及 "Slack" 时记录 Slack 活动
    * 当 `audio_index` 事件到达时总结讨论
    * 当特定关键词出现在 `transcript` 中时触发警报
    * 根据屏幕描述跟踪应用程序使用情况

11. **停止捕获**（完成后）—— 向捕获进程发送 SIGTERM。它应在其信号处理器中调用 `client.stop_capture()` 和 `client.shutdown()`。

12. **等待导出** —— 通过读取事件直到看到 `capture_session.exported`。此事件包含 `exported_video_id`、`stream_url` 和 `player_url`。这可能在停止捕获后需要几秒钟。

13. **停止 WebSocket 监听器**（在收到导出事件后）。使用 `kill $(cat /tmp/videodb_ws_pid)` 来干净地终止它。
...
在收到导出事件之前，请**不要**终止 WebSocket 监听器，否则您将错过最终的视频 URL。
