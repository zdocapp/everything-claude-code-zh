...
RTStream 支持实时摄取直播视频流（RTSP/RTMP）和桌面捕获会话。连接后，您可以录制、索引、搜索和导出直播源的内容。

有关代码级详细信息（SDK 方法、参数、示例），请参阅 [rtstream-reference.md](rtstream-reference.md)。

## 使用场景

* **安全与监控**：连接 RTSP 摄像头，检测事件，触发警报
