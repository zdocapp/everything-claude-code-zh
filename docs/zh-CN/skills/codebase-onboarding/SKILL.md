...
description: 分析一个陌生的代码库，并生成一份结构化的入门指南，包括架构图、关键入口点、约定规范以及一个初始的CLAUDE.md文件。适用于加入新项目或首次在仓库中设置Claude Code时使用。
origin: ECC
---

# 代码库入门指南

系统性地分析一个陌生的代码库，并生成结构化的入门指南。专为加入新项目的开发者或首次在现有仓库中设置 Claude Code 而设计。
...
在不阅读每个文件的情况下，收集关于项目的原始信号。并行运行以下检查：
...
4. 目录结构快照
   → 目录树的前 2 级，忽略 node_modules, vendor,
...
* 根据最近提交推断提交信息风格
* PR 工作流（压缩合并、合并、变基）
* 如果仓库尚无提交记录或历史记录过浅（例如 `git clone --depth 1`），则跳过此部分并注明“Git 历史记录不可用或过浅，无法检测规范”
...
- **UI 页面**：`src/app/(dashboard)/` — 已认证页面
- **数据库**：`prisma/schema.prisma` — 数据模型单一事实来源
- **配置**：`next.config.ts` — 构建与运行时配置
...
根据检测到的规范，生成或更新项目特定的 CLAUDE.md。如果 `CLAUDE.md` 已存在，请先读取它并进行增强——保留现有的项目特定说明，并清晰标注新增或更改的内容。
...
2. **验证而非猜测** —— 如果从配置中检测到某个框架，但实际代码使用了其他东西，请以代码为准。
3. **尊重现有的 CLAUDE.md** —— 如果已存在，请增强它而非替换它。标注出新内容与现有内容的区别。
4. **保持简洁** —— 入门指南应在 2 分钟内可浏览完毕。细节应留在代码中，而非指南里。
5. **标记未知项** —— 如果无法自信地检测到某个规范，请说明情况而非猜测。“无法确定测试运行器”比给出错误答案更好。

## 应避免的反模式

* 生成超过 100 行的 CLAUDE.md —— 保持其聚焦
* 列出每个依赖项 —— 仅突出那些影响你编写代码方式的依赖
* 描述显而易见的目录名称 —— `src/` 不需要解释
