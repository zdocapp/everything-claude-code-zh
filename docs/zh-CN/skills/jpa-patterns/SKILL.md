---
name: jpa-patterns
description: Spring Boot 中实体设计、关系、查询优化、事务、审计、索引、分页和连接池的 JPA/Hibernate 模式。
origin: ECC
---
...
* 对于读取频繁的实体，谨慎考虑二级缓存；验证其驱逐策略

## 迁移

* 使用 Flyway 或 Liquibase；切勿在生产中依赖 Hibernate 自动 DDL
* 保持迁移是幂等的和增量的；避免无计划地删除列
...
**记住**：保持实体精简、查询意图明确、事务简短。使用获取策略和投影来预防 N+1 问题，并为你的读写路径建立索引。
