...
你正在分析一个编码智能体（Claude Code）的技能/规则文件。
你的任务：提取当此技能激活时，智能体应遵循的**可观察行为序列**。
...
* id: \<snake\_case>
  ...
  required: true|false
  detector:
  description: <描述要查找的工具调用的自然语言描述>
  after\_step: <此步骤必须在其后执行的步骤 id，可选 — 如不需要则省略>
  before\_step: <此步骤必须在其前执行的步骤 id，可选 — 如不需要则省略>
...
* 仅当技能说明中提到"可选地"或"如果适用"时，才将步骤标记为 required: false
* 3-7 个步骤为理想范围。不要过度分解
