...
description: 针对离散和批量制造的生产调度、作业排序、产线平衡、换模优化和瓶颈解决的专业知识编码。基于拥有15年以上经验的生产调度员的知识。包括约束理论/鼓-缓冲-绳、快速换模、设备综合效率分析、中断响应框架以及企业资源计划/制造执行系统交互模式。适用于生产调度、解决瓶颈、优化换模、响应中断或平衡制造产线时使用。license: Apache-2.0
version: 1.0.0
