...
**优点：** 在全量推出前，通过真实流量发现问题
**缺点：** 需要流量分割基础设施和监控
**适用场景：** 高流量服务，高风险更改，功能标志
...
- 以非 root 用户身份运行
- 优先复制依赖文件（利用层缓存）
- 使用 .dockerignore 排除 node_modules、.git、tests 等目录
- 添加 HEALTHCHECK 指令
...
* \[ ] 公共端点已启用速率限制
* \[ ] 身份验证和授权已验证
* \[ ] 安全标头已设置（CSP、HSTS、X-Frame-Options）
