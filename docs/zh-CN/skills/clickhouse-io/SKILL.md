...
适用于 ClickHouse 的高性能分析和数据工程模式。

## 何时启用

* 设计 ClickHouse 表结构时（选择 MergeTree 引擎）
* 编写分析查询时（聚合、窗口函数、连接）
* 优化查询性能时（分区剪枝、投影、物化视图）
* 摄取大量数据时（批量插入、Kafka 集成）
* 为分析工作负载从 PostgreSQL/MySQL 迁移到 ClickHouse 时
* 实现实时仪表板或时间序列分析时
...
* 使用最合适的较小类型（例如 UInt32 而非 UInt64）
...
**请记住**：ClickHouse 擅长分析型工作负载。根据您的查询模式设计表，进行批量插入，并利用物化视图进行实时聚合。
