---
name: perl-testing
description: 使用Test2::V0、Test::More、prove运行器、模拟、Devel::Cover覆盖率以及TDD方法的Perl测试模式。
origin: ECC
---

# Perl 测试模式

使用 Test2::V0、Test::More、prove 和 TDD 方法论的 Perl 应用程序综合测试策略。
...
将可重用的辅助函数放在 `t/lib/TestHelper.pm` 中，并使用 `use lib 't/lib'` 加载。通过 `Exporter` 导出工厂函数，如 `create_test_db()`、`create_temp_dir()` 和 `fixture_path()`。
...
对于轻量级的模拟对象，使用 `Test::MockObject` 创建可注入的测试替身，使用 `->mock()` 并调用 `->called_ok()` 来验证调用。
...
模拟*依赖项*，而不是被测试的代码。如果你的测试只验证模拟返回了你告诉它的内容，那么它什么也没测试。
