...
description: "开源管道：分叉、清理和打包私有项目，以便安全地公开发布。链接3个代理（分叉器、清理器、打包器）。触发词：'/opensource'、'open source this'、'make this public'、'prepare for open source'。"
origin: ECC
---

# 开源管道技能

通过一个三阶段管道安全地开源任何项目：**Fork**（剥离密钥）→ **Sanitize**（验证清理）→ **Package**（CLAUDE.md + setup.sh + README）。

## 何时激活

* 用户说“开源这个项目”或“将其公开”
* 用户希望将私有仓库准备为公开发布
* 用户在推送到 GitHub 前需要剥离密钥
...
| 命令 | 操作 |
|---------|--------|
| `/opensource fork PROJECT` | 完整管道：fork + sanitize + package |
...
```
Agent(
  description="为开源项目 {PROJECT} 创建分支",
...
```
Agent(
  description="为 {PROJECT} 项目进行开源打包",
...
显示每个项目及其管道进度（FORK\_REPORT.md、SANITIZATION\_REPORT.md、CLAUDE.md 的存在情况）。
...
* 对于新版本，始终运行完整管道（fork → sanitize → package）
* 暂存目录会一直保留，直到显式清理 — 可用于审查
* 在发布前，任何手动修复后重新运行清理器
* 将密钥参数化而非删除 — 保持项目功能

## 相关技能

查看 `security-review` 以了解清理器使用的密钥检测模式。
