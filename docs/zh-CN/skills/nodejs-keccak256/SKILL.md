...
以太坊使用的是 Keccak-256，而非 Node.js `crypto.createHash('sha3-256')` 所暴露的 NIST 标准化 SHA3 变体。

## 何时使用

* 计算以太坊函数选择器或事件主题
* 在 JS/TS 中构建 EIP-712、签名、默克尔树或存储槽辅助工具
* 审查任何直接使用 Node crypto 对以太坊数据进行哈希处理的代码

## 工作原理

这两种算法对相同的输入会产生不同的输出，而 Node.js 不会发出警告。
...
在以太坊上下文中，切勿使用 `crypto.createHash('sha3-256')`。应使用来自 `ethers`、`viem`、`web3` 或其他明确 Keccak 实现的、支持 Keccak 的辅助工具。
