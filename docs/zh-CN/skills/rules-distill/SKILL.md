...
扫描已安装的技能，提取出现在多个技能中的横切原则，并将其提炼成规则——追加到现有规则文件、修订过时内容或创建新的规则文件。

应用“确定性收集 + LLM 判断”原则：脚本详尽地收集事实，然后由 LLM 通读完整上下文并给出裁决。
...
────────────────────────────────────────
技能：扫描了 {N} 个文件
规则：索引了 {M} 个文件（包含 {K} 个标题）

正在进入交叉阅读分析...
```

### 阶段 2：通读、匹配与裁决（LLM 判断）

提取和匹配在单次处理中统一完成。规则文件足够小（总计约 800 行），可以将全文提供给 LLM——无需 grep 预过滤。

#### 分批处理

根据技能描述将其分组到**主题集群**中。在子代理中分析每个集群，并提供完整的规则文本。
...
* 对相同或重叠原则的候选规则进行去重
* 使用**所有**批次合并的证据重新检查“2+ 技能”要求——在每个批次中出现在 1 个技能中，但总计出现在 2+ 技能中的原则是有效的
...
````
你是一位分析师，负责交叉阅读技能文档，提取应提升为规则的原则。
...
仅当满足以下所有条件时，才包含一个候选原则：

1.  **出现在 2+ 个技能中**：仅在一个技能中发现的原则应保留在该技能中
2.  **可操作的行为改变**：可以写成“做 X”或“不要做 Y”的形式——而不是“X 很重要”
3.  **明确的违规风险**：如果忽略此原则，会出现什么问题（1 句话）
4.  **尚未存在于规则中**：检查完整的规则文本——包括用不同词语表达的概念

## 匹配与判定

对于每个候选原则，对照完整的规则文本进行比较并给出判定：
...
```json
{
  ...
  "draft": "针对 Append/New Section/New File 判定的草案文本",
  "revision": {
    "reason": "为什么现有内容不准确或不充分（仅限 Revise）",
    "before": "待替换的当前文本（仅限 Revise）",
    "after": "提议的替换文本（仅限 Revise）"
  }
}
```
...
|---------|---------|-------------------|
| **追加** | 添加到现有章节 | 目标 + 草案 |
| **修订** | 修正不准确/不充分的内容 | 目标 + 原因 + 修订前/后 |
| **新章节** | 在现有文件中添加新章节 | 目标 + 草案 |
| **新文件** | 创建新的规则文件 | 文件名 + 完整草案 |
| **已涵盖** | 规则中已涵盖（可能措辞不同） | 原因（1 行） |
| **过于具体** | 应保留在技能中 | 相关技能的链接 |
...
* **候选 ID 格式**：基于原则的 kebab-case（例如，`llm-output-trust-boundary`）
...
[子代理分析：批次 1 (agent/meta 技能) ...]
[子代理分析：批次 2 (coding/pattern 技能) ...]
[跨批次合并：移除 2 个重复项，提升 1 个跨批次候选]
...
违规风险：LLM 输出中的格式漂移、类型不匹配或语法错误导致下游处理崩溃
草案：
  ## LLM 输出验证
  在重用 LLM 输出前，进行规范化、类型检查和清理...
...
✓ 已应用：performance.md §Context Window Management
✗ 已跳过：迭代边界
✗ 已跳过：边界类型转换
...
* **防过度抽象保障**：三层过滤器（2+ 技能证据、可操作行为测试、违规风险）防止过于抽象的原则进入规则。
