...
description: 基于C++核心指南（isocpp.github.io）的C++编码标准。在编写、审查或重构C++代码时使用，以强制执行现代、安全和惯用的实践。
origin: ECC
---

# C++ 编码标准（C++ 核心准则）

源自 [C++ 核心准则](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines) 的现代 C++（C++17/20/23）综合编码标准。强制类型安全、资源安全、不变性和清晰性。
...
* 特定准则与硬件约束冲突的嵌入式/裸机环境（选择性适配）

## 贯穿性原则

这些主题在整个准则中反复出现，并构成基础：

1. **处处使用 RAII**（P.8, R.1, E.6, CP.20）：将资源生命周期绑定到对象生命周期
2. **默认不变性**（P.10, Con.1-5, ES.25）：从 `const`/`constexpr` 开始；可变性是例外
...
| **F.6** | 如果你的函数必须不抛出异常，将其声明为 `noexcept` |
| **F.8** | 优先纯函数 |
| **F.16** | 对于“输入”参数，廉价复制类型按值传递，其他类型通过 `const&` 传递 |
...
| **C.9** | 最小化成员的暴露 |
| **C.20** | 如果可以避免定义默认操作，就避免（零法则） |
| **C.21** | 如果你定义或 `=delete` 任何拷贝/移动/析构函数，处理所有相关操作（五法则） |
...
| **Con.4** | 对于构造后不改变的值，使用 `const` |
| **Con.5** | 对于可在编译时计算的值，使用 `constexpr` |
...
| **CP.20** | 使用 RAII，永远不要使用普通的 `lock()`/`unlock()` |
...
* 在全局作用域的头部使用 `using namespace std;` (SF.7)
* 依赖包含顺序的头部文件 (SF.10, SF.11)
* 匈牙利命名法，如 `strName`、`iCount` (NL.5)
* 对宏以外的任何内容使用全大写字母 (NL.9)
...
* \[ ] 使用 `enum class` 而非普通的 `enum` (Enum.3)
* \[ ] 使用 `nullptr` 而非 `0`/`NULL` (ES.47)
* \[ ] 没有窄化转换 (ES.46)
* \[ ] 没有 C 风格的类型转换 (ES.48)
* \[ ] 单参数构造函数是 `explicit` (C.46)
* \[ ] 应用了零法则或五法则 (C.20, C.21)
* \[ ] 基类析构函数是公开虚函数或受保护非虚函数 (C.35)
* \[ ] 模板使用概念进行约束 (T.10)
* \[ ] 头部文件的全局作用域中没有 `using namespace` (SF.7)
* \[ ] 头部文件有包含保护且是自包含的 (SF.8, SF.11)
