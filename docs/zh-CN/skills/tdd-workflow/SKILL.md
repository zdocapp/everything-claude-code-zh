---
name: tdd-workflow
description: 在编写新功能、修复错误或重构代码时使用此技能。强制执行测试驱动开发，确保80%以上的覆盖率，包括单元测试、集成测试和端到端测试。
origin: ECC
---

# 测试驱动开发工作流

此技能确保所有代码开发遵循 TDD 原则，并具备全面的测试覆盖率。
...
* 首选的紧凑工作流是：
  ...
  * 一个可选的提交用于完成重构
* 如果测试提交明确对应 RED 状态，修复提交明确对应 GREEN 状态，则不需要单独的仅用于证据的提交
...
```
作为一名[角色]，我希望[行动]，以便[获益]

示例：
作为一名用户，我希望能够进行语义化市场搜索，
这样即使没有准确的关键词，我也能找到相关的市场。
```
...
* 编译时 RED：
  ...
  * 编译失败本身就是预期的 RED 信号
* 无论哪种情况，失败都是由预期的业务逻辑缺陷、未定义行为或缺失实现引起的
...
如果仓库使用 Git，请在此阶段验证后立即创建一个检查点提交。
推荐的提交信息格式：

* `test: add reproducer for <feature or bug>`
* 如果重现器已编译并执行，并因预期原因失败，此提交也可作为 RED 验证检查点
...
如果仓库使用 Git，现在暂存最小修复，但将检查点提交推迟到步骤 5 中验证 GREEN 状态之后。
...
如果仓库使用 Git，请在验证 GREEN 状态后立即创建一个检查点提交。
推荐的提交信息格式：
...
如果仓库使用 Git，请在重构完成且测试保持通过后立即创建一个检查点提交。
推荐的提交信息格式：
...
4. **准备-执行-断言** - 清晰的测试结构
5. **模拟外部依赖** - 隔离单元测试
6. **测试边界情况** - 空值、未定义、空值、大值
7. **测试错误路径** - 不仅仅是正常路径
8. **保持测试快速** - 每个单元测试 < 50 毫秒
9. **测试后清理** - 无副作用
10. **审查覆盖率报告** - 识别差距
...
**请记住**：测试不是可选的。它们是安全网，能够实现自信的重构、快速的开发和生产的可靠性。
