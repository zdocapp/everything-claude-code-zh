---
name: perl-patterns
description: 现代 Perl 5.36+ 的惯用法、最佳实践和约定，用于构建健壮、可维护的 Perl 应用程序。
origin: ECC
---
...
将这些模式作为偏向现代 Perl 5.36+ 默认设置的准则来应用：签名、显式模块、聚焦的错误处理和可测试的边界。下面的示例旨在作为起点供复制，然后根据你面前的实际应用程序、依赖栈和部署模型进行调整。

## 核心原则

### 1. 使用 `v5.36` 编译指令

一个 `use v5.36` 即可替代旧的样板代码，并启用严格模式、警告和子程序签名。
...
优先使用 Moo 进行轻量级、现代的面向对象编程。仅在需要其元协议时才使用 Moose。
...
**记住**：现代 Perl 是简洁、可读且安全的。让 `use v5.36` 处理样板代码，使用 Moo 处理对象，并优先选择 CPAN 上经过实战检验的模块，而不是自己手写的解决方案。
