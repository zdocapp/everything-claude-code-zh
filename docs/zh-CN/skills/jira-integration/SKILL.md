...
直接从您的 AI 编码工作流中检索、分析和更新 Jira 工单。支持 **基于 MCP 的**（推荐）和 **直接 REST API** 两种方法。
...
安装 `mcp-atlassian` MCP 服务器。这将 Jira 工具直接暴露给您的 AI 代理。

**要求:**

* Python 3.10+
* `uvx`（来自 `uv`），通过您的包管理器或官方 `uv` 安装文档安装
...
> **安全提示:** 切勿硬编码密钥。建议在系统环境（或密钥管理器）中设置 `JIRA_URL`、`JIRA_EMAIL` 和 `JIRA_API_TOKEN`。仅将 MCP `env` 块用于本地、未提交的配置文件。
...
如果 MCP 不可用，请通过 `curl` 或辅助脚本直接使用 Jira REST API v3。
...
将这些存储在您的 shell 环境、密钥管理器或未跟踪的本地环境文件中。不要将它们提交到仓库。
...
| 测试编写完成 | 评论附带测试覆盖率摘要 |
| 分支创建完成 | 评论附带分支名称 |
| PR/MR 创建完成 | 评论附带链接，链接问题 |
| 测试通过 | 评论附带结果摘要 |
| PR/MR 合并完成 | 转换到"已完成"或"评审中" |
...
* **验证** 在发起 API 调用前是否已设置凭据 — 快速失败并给出明确消息
