...
description: "自主Claude代码循环的模式与架构——从简单的顺序流水线到RFC驱动的多智能体DAG系统。"
origin: ECC
---
...
> 规范的技能名称现在是 `continuous-agent-loop`。新的循环指南应在此处编写，而此技能仍可用以避免破坏现有工作流。

用于在循环中自主运行 Claude Code 的模式、架构和参考实现。涵盖从简单的 `claude -p` 流水线到完整的 RFC 驱动的多智能体 DAG 编排的一切。
...
| 模式 | 复杂度 | 最适合 |
|---------|-----------|----------|
| [顺序流水线](#1-顺序流水线-claude--p) | 低 | 日常开发步骤，脚本化工作流 |
| [NanoClaw REPL](#2-nanoclaw-repl) | 低 | 交互式持久会话 |
| [无限智能体循环](#3-无限智能体循环) | 中 | 并行内容生成，规范驱动的工作 |
| [持续 Claude PR 循环](#4-持续-claude-pr-循环) | 中 | 带有 CI 门控的多天迭代项目 |
| [去草率模式](#5-去草率模式) | 附加 | 在任何实现者步骤之后进行质量清理 |
...
**最简单的循环。** 将日常开发分解为一系列非交互式的 `claude -p` 调用。每次调用都是一个具有明确提示的专注步骤。

### 核心见解

> 如果你无法设计出这样的循环，那意味着你甚至无法在交互模式下驱动 LLM 来修复你的代码。

`claude -p` 标志使用提示非交互式地运行 Claude Code，完成后退出。链接调用以构建流水线：
...
1. **每个步骤都是隔离的** — 每个 `claude -p` 调用都有一个新的上下文窗口，意味着步骤之间没有上下文泄漏。
2. **顺序很重要** — 步骤按顺序执行。每个步骤都建立在前一个步骤留下的文件系统状态之上。
3. **否定性指令是危险的** — 不要说“不要测试类型系统”。相反，添加一个单独的清理步骤（参见[去草率模式](#5-去草率模式)）。
4. **退出代码会传播** — `set -e` 在失败时停止流水线。
...
**ECC 内置的持久循环。** 一个具有会话感知能力的 REPL，使用完整的对话历史同步调用 `claude -p`。
...
2. 每条用户消息都随完整历史作为上下文发送到 `claude -p`
...
阶段 3：规划创意方向 — 每个代理获得一个不同的主题/方法。
阶段 4：并行部署子代理（任务工具）。每个代理接收：
  - 完整的规范文本
  - 当前目录快照
  - 它们分配的迭代编号
  - 它们独特的创意方向
...
不要依赖智能体自我区分。编排器**分配**每个智能体一个特定的创意方向和迭代编号。这可以防止并行智能体之间出现重复概念。
...
## 进度
- [x] 已添加认证模块测试（迭代 1）
- [x] 已修复令牌刷新中的边界情况（迭代 2）
- [ ] 仍需完成：限流测试、错误边界测试

## 后续步骤
- 接下来专注于限流模块
- 测试/helpers.ts 中的模拟设置可以复用
```

Claude 在迭代开始时读取此文件，并在迭代结束时更新它。这弥合了独立的 `claude -p` 调用之间的上下文差距。
...
当你要求 LLM 使用 TDD 实现时，它对“编写测试”的理解过于字面化：
...
> 与其添加具有下游质量影响的否定性指令，不如添加一个单独的去草率阶段。两个专注的智能体优于一个受限制的智能体。

***

## 6. Ralphinho / RFC 驱动的 DAG 编排

**最复杂的模式。** 一个 RFC 驱动的多智能体流水线，将规范分解为依赖关系 DAG，通过分层质量流水线运行每个单元，并通过智能体驱动的合并队列落地。由 enitrat 创建（致谢：@enitrat）。
...
* 倾向于更少、内聚的单元（最小化合并风险）
* 最小化跨单元文件重叠（避免冲突）
* 将测试与实现保持在一起（永远不要分离“实现 X” + “测试 X”）
* 仅在实际存在代码依赖关系的地方建立依赖关系
...
| Final Review | Opus | 质量门控（仅限 large 层级） |

**关键设计：** 审查者从未编写过它审查的代码。这消除了作者偏见——这是自我审查中最常见的遗漏问题来源。
...
**驱逐恢复：**
当被驱逐时，捕获完整上下文（冲突文件、差异、测试输出）并在下一次 Ralph 传递时反馈给实现者：

```markdown
## 合并冲突 — 在下一次提交前解决

您之前的实现与另一个先提交的单元发生了冲突。
请调整您的更改，以避免以下冲突的文件/行。
...
implement.{filesCreated, whatWasDone} ─────→ 测试, 评审
test.failingSummary ───────────────────────→ 评审, 实施 (下一轮)
reviews.{feedback, issues} ────────────────→ 评审修复 → 实施 (下一轮)
...
同一单元的流水线阶段**共享**一个工作树，在研究 → 计划 → 实现 → 测试 → 审查之间保留状态（上下文文件、计划文件、代码更改）。

### 关键设计原则

1. **确定性执行** — 预先分解锁定并行性和顺序
2. **在杠杆点进行人工审查** — 工作计划是单一最高杠杆干预点
3. **关注点分离** — 每个阶段在独立的上下文窗口中使用独立的智能体
4. **带上下文的冲突恢复** — 完整的驱逐上下文支持智能重新运行，而非盲目重试
5. **层级驱动的深度** — 琐碎更改跳过研究/审查；大型更改获得最大程度的审查
...
1. **顺序流水线 + 去杂乱化** — 最常见的组合。每个实现步骤都进行一次清理。

2. **持续 Claude + 去杂乱化** — 在每次迭代中添加 `--review-prompt` 并附带去杂乱化指令。

3. **任何循环 + 验证** — 在提交前使用 ECC 的 `/verify` 命令或 `verification-loop` 技能作为关卡。

4. **在简单循环中使用 Ralphinho 的分层方法** — 即使在顺序流水线中，你也可以将简单任务路由到 Haiku，复杂任务路由到 Opus：
   ```bash
   # 简单的格式化修复
...
2. **迭代之间没有上下文桥接** — 每次 `claude -p` 调用都是全新的开始。使用 `SHARED_TASK_NOTES.md` 或文件系统状态来桥接上下文。

3. **重试相同的失败** — 如果一次迭代失败，不要只是重试。捕获错误上下文并将其提供给下一次尝试。

4. **使用负面指令而非清理步骤** — 不要说“不要做 X”。添加一个单独的步骤来移除 X。

5. **所有智能体都在同一个上下文窗口中** — 对于复杂的工作流，将关注点分离到不同的智能体进程中。审查者不应是作者。
