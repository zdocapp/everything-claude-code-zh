...
使用冻结的数据类跟踪累计支出。每次 API 调用返回一个新的跟踪器 —— 永不改变状态。
...
* **在处理批次之前设置明确的预算限制** —— 提前失败而非超支
* **记录模型选择决策**，以便您可以根据真实数据调整阈值
* **对超过 1024 个令牌的系统提示词使用提示词缓存** —— 既节省成本又降低延迟
* **切勿在身份验证或验证错误时重试** —— 仅针对瞬时故障（网络、速率限制、服务器错误）进行重试
