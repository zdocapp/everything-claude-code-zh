...
针对现代 C++（C++17/20）使用 GoogleTest/GoogleMock 与 CMake/CTest 的、以代理为中心的测试工作流。
...
* 保持测试的确定性和隔离性
* 优先使用依赖注入而非全局变量
* 使用 `ASSERT_*` 表示前置条件，`EXPECT_*` 表示多重检查
* 在 CTest 标签或目录中分离单元测试与集成测试
* 在 CI 中运行消毒工具以检测内存和竞争问题

### 不应该做

* 不要在单元测试中依赖真实时间或网络
* 当可以使用条件变量时，不要使用休眠作为同步手段
* 不要过度模拟简单的值对象
* 不要对非关键日志使用脆弱的字符串匹配
...
* **依赖挂钟时间** → 注入时钟或使用模拟时间源。
* **偶发性并发测试** → 使用条件变量/闩锁和有界等待。
* **隐藏的全局状态** → 在夹具中重置全局状态或移除全局变量。
* **过度模拟** → 对有状态行为优先使用仿制品，仅对交互进行模拟。
* **缺少消毒工具运行** → 在 CI 中添加 ASan/UBSan/TSan 构建。
* **仅在调试构建上计算覆盖率** → 确保覆盖率目标使用一致的标志。
