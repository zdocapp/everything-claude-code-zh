...
自主操作必须由用户明确请求并限定范围。除非用户已批准该能力且当前设置的目标工作区已获授权，否则不得创建计划、调度远程代理、写入持久内存、使用计算机控制、对外发布、修改第三方资源或对私人通信采取行动。

在启用定期或事件驱动操作前，优先使用模拟运行计划和本地队列文件。请将凭证、私有工作区导出文件、个人数据集及账户特定自动化内容排除在可复用的ECC制品之外。
...
远程触发 Claude Code 代理以实现事件驱动的工作流。
