...
创建 **CodeTour** `.tour` 文件，用于代码库的逐步讲解，可直接打开真实文件和行范围。导览文件存放在 `.tours/` 中，专为 CodeTour 格式设计，而非临时 Markdown 笔记。
...
| 任务是实现或重构 | 执行实现工作 |
| 任务是不需要导览产物的广泛代码库入职 | `codebase-onboarding` |
...
| 用户画像不匹配 | 为实际读者而非通用工程师编写 |

## 最佳实践

* 使步骤数量与仓库大小和用户画像深度成比例
* 使用目录步骤进行定位，使用文件步骤进行实质讲解
* 对于 PR 导览，首先覆盖变更的文件
* 对于单体仓库，将范围限定在相关包，而非遍历所有内容
* 以读者现在可以做什么结束，而非总结回顾
