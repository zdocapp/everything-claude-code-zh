---
name: hermes-imports
description: 将本地 Hermes 操作员工作流转换为经过清理的 ECC 技能和发布包工件。在准备 Hermes 工作流以供公共 ECC 重用而不泄露私有工作区状态、凭据或仅本地路径时使用。
origin: ECC
---
...
Hermes 是操作员 shell。ECC 是可复用工作流层。导入操作应将稳定模式从 Hermes 迁移至 ECC，同时避免移动私有状态。

## 使用时机

* Hermes 工作流重复次数足够多，已具备可复用性
* 本地操作员提示词需升级为公共 ECC 技能
...
返回一条 X 帖子、一条 LinkedIn 帖子、一份录制检查清单以及缺失的资产列表。
```
