...
自动评估 Claude Code 会话的结尾，以提取可重用的模式，这些模式可以保存为已学习的技能。

## 何时激活

* 设置从 Claude Code 会话自动提取模式时
* 为会话评估配置 Stop 钩子时
* 在 `~/.claude/skills/learned/` 中查看或管理已学习的技能时
* 调整提取阈值或模式类别时
* 比较 v1（当前版本）与 v2（基于本能）的方法时

## 状态

此 v1 技能仍受支持，但 `continuous-learning-v2` 是新安装的首选路径。当你明确需要更简单的 Stop-hook 提取流程，或需要与旧的已学习技能工作流兼容时，请保留 v1。

## 工作原理

此技能在每次会话结束时作为 **Stop 钩子** 运行：

1. **会话评估**：检查会话是否有足够多的消息（默认：10 条以上）
2. **模式检测**：从会话中识别可提取的模式
...
|---------|--------------|---------------|
| 观察 | Stop 钩子（会话结束时） | PreToolUse/PostToolUse 钩子（100% 可靠） |
| 分析 | 主上下文 | 后台代理（Haiku） |
| 粒度 | 完整技能 | 原子化的“本能” |
| 置信度 | 无 | 0.3-0.9 加权 |
